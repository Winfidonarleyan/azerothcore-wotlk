...
void AuraEffect::HandlePeriodicHealthLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    caster->GetThreatMgr().ForwardThreatForAssistingMe(caster, float(gain) / 2, GetSpellInfo(), true);
}
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (caster)
        target->GetThreatMgr().ForwardThreatForAssistingMe(caster, float(gain) / 2, GetSpellInfo());
...
void AuraEffect::HandlePeriodicManaLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    if (gainAmount)
    {
        ...
        target->GetThreatMgr().AddThreat(caster, float(gainedAmount) * 0.5f, GetSpellInfo(), GetSpellInfo(), true);
    }
...
void AuraEffect::HandleObsModPowerAuraTick(Unit* target, Unit* caster) const
{
    ...
    if (caster)
        target->GetThreatMgr().ForwardThreatForAssistingMe(caster, float(gain) / 2, GetSpellInfo(), true);
}
